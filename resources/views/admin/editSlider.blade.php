...
@section('content')
        ...
        <div class="modal-dialog">
            ...
            <div class="modal-content">
                    ...
                    <form id="pictureForm" enctype="multipart/form-data">
                        ...
                        @if($slider->image_src != null)
                            <img class="image" id="editable"
                                 ...
                                 src="{{url('public/dashboard/sliderImages')}}/{{$slider->image_src}}">
                        @endif
                        @if($slider->image_src == null)
                ...
                <div class="modal-footer" >
                    ...
                    <button type="button" content="{{$slider->id}}" id="editSliderPicture" class="btn btn-warning col-md-5" >ویرایش</button>
                </div>
            </div>
    ...
    <div class="row">
                            ...
                            ({
                                ...
                                beforeSend: function () {
                                    if ($('#title').val() == null || $('#title').val() == '') {
                                        ...
                                        swal({
                                            title: "",
                                            text: 'پر کردن عنوان اسلایدر الزامی است',
                                            type: "warning",
        ...
        <script>
            $(document).on('click','#editSliderPicture',function(){
                var formData = new FormData($('#pictureForm')[0]);
                $.ajax
                ({
                    cache        : false,
                    url          : "{{url('admin/editSliderPicture')}}",
                    type         : "post",
                    data         : formData,
                    dataType     : "JSON",
                    processData  : false,
        ...
        <script>
            $(document).on('click','#active',function () {
                ...
                //alert(title);
                swal({
                        title:   " آیا در نظر دارید وضعیت اسلایدر را " +"(( "+ title +" ))"+  "  نمایید؟ ",
                        text: "",
                        type: "info",
                        showCancelButton: true,
                        confirmButtonColor: "	#5cb85c",
                    ...
                    function (isConfirm) {
                        if (isConfirm) {
                            ...
                            ({
                                cache: false,
                                url: "{{url('admin/enableOrDisableSlider')}}",
                                type: "post",
                                data: {'_token': token, 'active': active, 'sliderId': sliderId},
                                dataType: "JSON",
                                success: function (response) {
                                    if (response.code == 1) {
                                        swal({
